...
public class DAGLongestPath {
    ...
    public static Result longestFromAutoSources(Graph dag, Map<String, Integer> nodeDurations) {
        ...
        for (String n : dag.getNodes()) dist.put(n, Double.NEGATIVE_INFINITY);

        boolean useNodeDurations = nodeDurations != null && !nodeDurations.isEmpty();


        if (useNodeDurations) {
            for (String s : sources) {
                double d = nodeDurations.getOrDefault(s, 0);
                dist.put(s, d);
                parent.put(s, null);
            }
        } else {
        ...
        for (String u : topo) {
            ...
            for (Edge e : adj.getOrDefault(u, Collections.emptyList())) {
                ...
                double candidate;
                if (useNodeDurations) {
                    double durV = nodeDurations.getOrDefault(v, 0);
                    candidate = du + durV;
                } else {
                    candidate = du + e.weight;
                }
                if (candidate > dist.getOrDefault(v, Double.NEGATIVE_INFINITY)) {
